...
def resample(N, particles, weights_and_index):
    new_particles = []
    weights = []

    # Obtenir els pesos del pesat original
    for w in weights_and_index:
        weights.append(w[0])

    if sum(weights) == 0:
        # Si els pesos sumen zero, es generen partícules completament noves
        for _ in range(N):
            px = random.uniform(0, 9)
            py = random.uniform(0, 3)
            ori = random.uniform(0, 2 * math.pi)
            new_particles.append([px, py, ori])
    ...
    # Càlcul del re-sampling amb reemplaçament
    cumulative_weights = np.cumsum(normalized_weights)
    cumulative_weights = np.sort(cumulative_weights,axis=0)
    for _ in range(N):
        # Punt aleatori per a selecció de partícules
        r = random.random()
        selected_index = 0

        # Trobar l'índex corresponent al punt aleatori
        for pr, cumulative_weight in enumerate(cumulative_weights):
            if r < cumulative_weight:
                selected_index = pr
                break

        # Generar noves partícules amb una mica de variació aleatòria
        while True:
            part = particles[selected_index]
            new_px = random.normalvariate(mu=part[0], sigma=0.05)
            new_py = random.normalvariate(mu=part[1], sigma=0.05)
            new_ori = random.normalvariate(mu=part[2], sigma=np.pi / 20)

            # Comprovar si les noves coordenades són vàlides
            if 0 <= new_px <= 9 and 0 <= new_py <= 3:
                new_particles.append([new_px, new_py, new_ori])
                break

    return new_particles
def resample_old (N, particle, weights_and_index):
    new_particle = []
    weights = []
    ...
    if(sum(weights)==0):
        ...
        print("Cumulative Weights", cweights)
        # 20% will be retained
